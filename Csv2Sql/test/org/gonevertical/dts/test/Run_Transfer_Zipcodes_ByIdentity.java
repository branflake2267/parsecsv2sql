...
public class Run_Transfer_Zipcodes_ByIdentity {
  ...
  private void run() {
    ...
    toTable = "import_zipcodes_test_transfer_byidentity";
    
    // reset dest table
    //deleteDestRecords(dd_dst);
    
    ColumnData[] columnIdentities = new ColumnData[3];
		columnIdentities[0] = new ColumnData(fromTable, "zip_code", "varchar(255)");
		columnIdentities[1] = new ColumnData(fromTable, "latitude", "varchar(255)");
		columnIdentities[2] = new ColumnData(fromTable, "state_abbreviation", "varchar(255)");
    
    
    Transfer transfer = new Transfer(dd_src, dd_dst);
    transfer.setOffsetLimit(100); 
    transfer.setThreadsToSpawn(1);
